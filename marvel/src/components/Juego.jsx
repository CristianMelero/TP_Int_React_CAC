...
const [nombres_atributos_clickeados, setNombres_atributos_clickeados]=useState([]);
...
const submitHandler = (ev) => {
	...
	for (let i=0;i<22;i++){
		if (ev.target.avengers_clickeados[i].checked) {
			nombres_clickeados.push(data[ev.target.avengers_clickeados[i].value].Name);
		};
	};
	...
	for (let i=0;i<6;i++){
		if (ev.target.atributos_clickeados[i].checked) {
			nombres_atributos_clickeados.push(ev.target.atributos_clickeados[i].value);
		};
	};
	...
	return (
		...
		{/*//FORMULARIO*/}
				<form onSubmit={submitHandler}>
					...
					<div>
		   				<input name="avengers_clickeados" type="checkbox" value={element}/><label>{data[element].Name}</label>
					</div>   
					))}
        			...
        			<div>
            			<input name="atributos_clickeados" type="checkbox" value="Intelligence"/><label>Inteligencia</label>
        			</div>
        			<div>
            			<input name="atributos_clickeados" type="checkbox" value="Strength"/><label>Fuerza</label>
        			</div> 
        			<div>
            			<input name="atributos_clickeados" type="checkbox" value="Speed"/><label>Velocidad</label>
                	</div> 
        			<div>
            			<input name="atributos_clickeados" type="checkbox" value="Durability"/><label>Resistencia</label>
        			</div> 
        			<div>
			            <input name="atributos_clickeados" type="checkbox" value="Energy_Projection"/><label>Energía proyectada</label>
        			</div> 
        			<div> 
            			<input name="atributos_clickeados" type="checkbox" value="Fighting_Ability"/><label>Habilidad de combate</label>
        			</div>
					
					<button type="submit">Continuar</button>
					
					<p>Elegiste: {nombres_clickeados[0]} y {nombres_clickeados[1]}</p>
					<p>Elegiste: {nombres_atributos_clickeados[0]} y {nombres_atributos_clickeados[1]}</p>
